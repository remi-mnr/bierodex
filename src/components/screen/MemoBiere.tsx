  ...
  return (
    <div className="flex flex-col gap-8 p-6">
      <div className="relative h-10 w-full mt-4">
        <input placeholder="Outlined" onChange={(e) => setBite(e.target.value)}
      ...
      <Rank></Rank>
      <Slider className="mt-4 mb-4" defaultValue={[50]} max={100} step={1} value={[slide]} onValueChange={(v) => setSlide(v[0])} />
      <button className="btn btn-outline btn-primary" onClick={() => afficherNom(bite)}>Success</button>
    </div>
  );
}

export default MemoBiere;
